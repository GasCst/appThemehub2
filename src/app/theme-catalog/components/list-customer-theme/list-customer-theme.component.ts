import { UserDTO } from './../../../shared/model/response/UserDTO';
...
export class ListCustomerThemeComponent {
  ...
  calculateAverageRating(theme: ThemeDTO): number {
    ...
    const sum = theme.reviewList.reduce(
  ...
  async addToCart(theme: any) {
    console.log('Theme object:', theme);
    
    if (!theme || !theme.idtheme) {
      console.error('Theme ID is missing or undefined. Theme object:', theme);
      return;
    }
  
    console.log(`Adding theme with ID: ${theme.idtheme} to cart`);
  
    try {
      const response = await firstValueFrom(this._authService.AddToCart(theme.idtheme));
      ...
      console.error('Error adding theme to cart:', error);
    }
  }
  
  

  async buyNow(theme: any) {
    if (!this.ThemePurchase) {
      this.ThemePurchase = {
        themeId: 0,
        buyerId: 0,
        themesInPurchase: [],
      } as ThemePurchaseDTORequest;
      this.ThemesInPurchase = {
        id: 0,
        quantity: 0,
        theme: {} as ThemeDTO,
      } as ThemesInPurchaseDTO;
    }

    await firstValueFrom(this._authService.getBuyerId())
      .then((value: number) => (this.buyerId = value))
      .catch((error) => console.log(error + ' mmmmmmmmmmm '));

    this.ThemePurchase.buyerId = this.buyerId;
    ...
    await firstValueFrom(
      this._purchaseService.SavePurchaseTheme(this.ThemePurchase)
    );
    ...
    // da implementare redirect al checkout oppure fai altro.
  }
}
